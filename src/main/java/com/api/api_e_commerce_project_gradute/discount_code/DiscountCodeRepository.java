...
public interface DiscountCodeRepository extends JpaRepository<DiscountCode,Long> {
  ...
  @Query(value = "SELECT dc.* FROM discount_code dc  WHERE dc.code = ?1 AND dc.amount > 0 AND NOW() < time_expired "
          ,nativeQuery = true)
  ...
  @Query(value = "SELECT dc.* FROM discount_code dc INNER JOIN discount_code_user dcu " +
      "ON dc.id = dcu.id_discount_code WHERE dc.code = ?1 and dcu.id_user = ?2 AND dcu.is_used = 0 ",nativeQuery = true)
